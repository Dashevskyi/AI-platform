...
ЖЕЛЕЗНЫЕ ПРАВИЛА:
1. НИКОГДА не угадывай имена таблиц/колонок. Сначала `list_data_sources`, затем
   `list_tables`, затем `list_columns` — смотри РЕАЛЬНЫЕ имена и типы.
...
- `default_limit`, `max_limit`, `sort_by`
...
async def builder_chat(
    ...
    return {"reply": "Достигнут лимит шагов. Уточни задачу или продолжи диалог.",
            "trace": trace, "proposed": proposed}
