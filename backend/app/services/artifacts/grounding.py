...
def format_active_artifacts_block(artifacts: list[Artifact]) -> str | None:
    ...
    parts: list[str] = [
        ...
        (
            ...
            "Для tool-result: это СНИМОК на момент «вызвано N мин назад», не "
            "текущее состояние. Вызови tool заново, если (а) метка > 2-3 минут "
            "и нужны актуальные данные, или (б) пользователь просит проверить/"
            "перепроверить/измерить ещё раз, или сообщил, что что-то изменил — "
            "тогда снимок устарел НЕЗАВИСИМО от возраста. Не выдавай старый "
            "снимок за новый замер и никогда не выдумывай результат измерения, "
            "которого не делал."
        ),
    ]
